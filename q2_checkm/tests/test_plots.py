...
class TestCheckMPlots(TestPluginBase):
    ...
    def assertPlot(self, obs_spec, exp_spec):
        ...
        if exp_spec.get("selection_count"):
            self.assertEqual(len(obs_spec.selection), exp_spec["selection_count"])
        else:
    ...
    def test_prep_scatter_plot(self):
        obs_spec = _prep_scatter_plot(
            ...
            x_title="Points",
            y_title="Metric1",
            primary_selection=self.selection,
            selection_col="bin:N",
            selection_title="Bin",
            color_map="blues",
            reverse_x=True,
    ...
    def test_prep_contig_plots(self):
        obs_specs = _prep_contig_plots(self.base_plot, self.selection, self.filter)
        exp_cols = {
